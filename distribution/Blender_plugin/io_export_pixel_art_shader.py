...
bl_info = {
    "name": "Pixel Art Shader Model Format (.x)",
    "author": "Toshihiro Kushizaki",
    "version": (1, 0, 0),
    "blender": (2, 69, 0),
    "location": "File > Export > Pixel Art Shader Model (.x)",
    "description": "Export Pixel Art Shader Model Format (.x)",
    "wiki_url": "",
...
class MeshExportObject(ExportObject):
    ...
    def __WriteMeshMaterials(self, Mesh):
        def WriteMaterial(Exporter, Material):
            ...
            Diffuse_Color = []
            Diffuse_Color.append( 0.0 )										# -
            Diffuse_Color.append( 0.0 )										# -
            Diffuse_Color.append( 0.0 )										# -
            Diffuse_Color.append( 0.0 )										# -

            Specular_Power = Material.specular_hardness						# Material No.

            Specular_Color = []
            Specular_Color.append( Material.specular_intensity )			    # specular coefficient
            Specular_Color.append( ( Material.specular_ior - 1.0 ) * 10.0 + 1 )	# specular exponent [1.0, 10.0] -> [1.0, 91.0]
            if Material.diffuse_fresnel < 0.1:
                Specular_Color.append( 0.0 )								# light map ONLY (off)
            else:
                Specular_Color.append( 1.0 )								# light map ONLY (on)

            Emissive_Color = []
            Emissive_Color.append( 0.0 )									# -
            Emissive_Color.append( 0.0 )									# -
            Emissive_Color.append( 0.0 )									# -

            Exporter.File.Write("{:9f};{:9f};{:9f};{:9f};;\n".format(Diffuse_Color[0], Diffuse_Color[1], Diffuse_Color[2], Diffuse_Color[3]))
            Exporter.File.Write(" {:9f};\n".format(Specular_Power))
            Exporter.File.Write("{:9f};{:9f};{:9f};;\n".format(Specular_Color[0], Specular_Color[1], Specular_Color[2]))
            Exporter.File.Write("{:9f};{:9f};{:9f};;\n".format(Emissive_Color[0], Emissive_Color[1], Emissive_Color[2]))
...
class ExportPixelArtShader(bpy.types.Operator):
    """Export to the Pixel Art Shader model format (.x)"""

    bl_idname = "export_scene.pixelartshader"
    bl_label = "Export Pixel Art Shader"
...
#        from . import export_x
        Exporter = PixelArtShaderExporter(self, context)
        Exporter.Export()
        return {'FINISHED'}
...
def menu_func(self, context):
    self.layout.operator(ExportPixelArtShader.bl_idname, text="Pixel Art Shader (.x)")
